...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!id || !title || !slug) {
    return NextResponse.redirect(new URL("/admin/pdfs?error=missing-fields", request.url), 303);
  }
  ...
  return NextResponse.redirect(new URL("/admin/pdfs?updated=1", request.url), 303);
}
