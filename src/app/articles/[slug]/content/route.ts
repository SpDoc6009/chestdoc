...
function injectResizeScript(html: string, articleId: string) {
  const style = `
<style>
  html,
  body {
...
<script>
(() => {
  const reportId = ${JSON.stringify(articleId)};
  let lastHeight = 0;
  function getHeight() {
...
export async function GET(_request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!article || (!article.isPublished && !isAdmin) || !article.htmlContent) {
    return new NextResponse("Not found", { status: 404 });
  }

  return new NextResponse(injectResizeScript(article.htmlContent, article.id), {
    headers: {
